...
// ============================================================
void workerThread(FrameQueue& workqueue, ResultQueue& resultQueue,
    ...
    while (true) {
        ...
        {
            std::unique_lock<std::mutex> lock(workqueue.mtx);
            workqueue.cv.wait(lock, [&] {
                return !workqueue.queue.empty() || workqueue.bitti;
            });
            if (workqueue.queue.empty() && workqueue.bitti) break;
            frame = workqueue.queue.front();
            workqueue.queue.pop();
        }
        ...
        {
            std::lock_guard<std::mutex> lock(resultQueue.mtx);
            resultQueue.queue.push(frame);
        }
        resultQueue.cv.notify_one();
    }
}
...
int main() {
    ...
    for (unsigned int i = 0; i < threadSayisi; i++) {
        threadHavuzu.push_back(std::thread(workerThread,
            std::ref(workqueue), std::ref(resultQueue),
            protoPath, modelPath));
    }
    ...
    while (true) {
        ...
        {
            std::lock_guard<std::mutex> lock(workqueue.mtx);
            workqueue.queue.push(hamKare.clone());
        }
        workqueue.cv.notify_one();

        // Worker bitene kadar bekle → ham yüz hiç görünmez
        cv::Mat islenmis;
        {
            std::unique_lock<std::mutex> lock(resultQueue.mtx);
            resultQueue.cv.wait(lock, [&] {
                return !resultQueue.queue.empty();
            });
            islenmis = resultQueue.queue.front();
    ...
    {
        std::lock_guard<std::mutex> lock(workqueue.mtx);
        workqueue.bitti = true;
    }
    workqueue.cv.notify_all();
